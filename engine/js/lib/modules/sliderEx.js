...
(function (f, define) {
    ...
    (function ($, undefined) {
        ...
        var SliderBase = Widget.extend({
            ...
            _setItemsTitle: function () {
                ...
                for (; i - limit !== 0 ; i += increment) {
                    $(items[i]).attr("title", format(options.tooltip.format, this.options.tooltip.transformer({ value: round(titleNumber) }).value));
            ...
            _getFormattedValue: function (val, drag) {
                ...
                if (isArray(val) || (drag && drag.type)) {
                    ...
                    if (tooltipTemplate) {
                        html = tooltipTemplate(this.options.tooltip.transformer({
                    ...
                    if (tooltipTemplate) {
                        ...
                        html = format(tooltip.format, this.options.tooltip.transformer({ value: val }).value);
                    }
                }
                return html;
            },
            ...
            _createHtml: function () {
                ...
                //Modified: By srinath to acheive custom filter track ui
        ...
        function createTrack(options, element) {
            ...
            var dg = ["<div class='k-slider-track'>"];
            dg.push("<div class='k-slider-selection'><!-- --></div>");
            if (dragHandleCount > 1) {
                dg.push("<a href='#' class='k-draghandle' title='" + firstDragHandleTitle + "' role='sliderex' aria-valuemin='" + options.min + "' aria-valuemax='" + options.max + "' aria-valuenow='");
                dg.push((options.selectionStart || options.min) + "'>Drag</a>");
                dg.push("<a href='#' class='k-draghandle' title='" + options.rightDragHandleTitle + "'role='sliderex' aria-valuemin='" + options.min + "' aria-valuemax='" + options.max + "' aria-valuenow='" + (options.selectionEnd || options.max) + "'>Drag</a>");
            }
            else {

                dg.push("<span class='k-draghandle sleek-thumb' title='" + firstDragHandleTitle + "' role='sliderex' aria-valuemin='" + options.min + "' aria-valuemax='" + options.max + "' aria-valuenow='" + (options.value || options.min) + "'>");
                dg.push("<a class='k-button pull-left k-button-decrease k-transparent'><span class='k-icon'");
                dg.push("' title='" + options["decreaseButtonTitle"] + "'>" + options["decreaseButtonTitle"] + "</span></a>");
                dg.push("<a class='pull-left k-button-dot'><span class='k-icon dot'></span></a>");
                dg.push("<a class='k-button pull-left k-button-increase k-transparent'><span class='k-icon'");
                dg.push("' title='" + options["increaseButtonTitle"] + "'>" + options["increaseButtonTitle"] + "</span></a>");
                dg.push("</span>");
            }
        ...
        var Slider = SliderBase.extend({
            ...
            options: {
                ...
                tooltip: {
                    enabled: true, format: "{0:#,#.##}", transformer: function (e) {
                        return { value: e.value, selectionStart: e.selectionStart, selectionEnd: e.selectionEnd };
                    }
                },
                value: null
            },
                ...
                if (options.showButtons) {
                    ...
                    that.wrapper.find(".k-button")
                        ...
                        .on(MOUSE_DOWN, proxy(function (e) {
                            //Modified: By Srinath to make slider move in negative direction
                        ...
                        .eq(1)
                        .on(MOUSE_DOWN, proxy(function (e) {
                            //Modified: By Srinath to make slider move in positive direction
                            mouseDownHandler(e, 1);
                        }, that))
