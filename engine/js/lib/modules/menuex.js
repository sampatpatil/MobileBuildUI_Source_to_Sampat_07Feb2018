    ...
    (function ($, undefined) {
        ...
        var Menu = Widget.extend({
            ...
            options: {
                ...
                //New functionalities feature
                menuTitleSelector: '.MenuText',
                expandedWidth: 205,
                collapsedWidth: 65,
                expandable: true,
            ...
            open: function (element, openImmediately) {
                var that = this,
                    options = that.options,
                ...
                element.each(function () {
                    ...
                    openImmediately ? openNow() : li.data(TIMER, setTimeout(openNow, that.options.hoverDelay));
                });

                return that;
            },

            close: function (items, dontClearClose, closeImmediately) {
                var that = this,
                ...
                items.each(function () {
                    ...
                    closeImmediately ? closeNow() : li.data(TIMER, setTimeout(closeNow, that.options.hoverDelay));
                });
            ...
            _mouseenter: function (e) {
                ...
                var executeTask = function () {
                    ...
                    if ((!that.options.openOnClick || that.clicked) && !touch) {
                        if (!contains(e.currentTarget, e.relatedTarget)) {//&& hasChildren) {
                            if (that.options.expandable && !that._isExpanded()) {
                                element.find(that.options.menuTitleSelector).fadeIn();
                                element.animate({ width: that.options.expandedWidth }, {
                                    complete: function () {
                                        element.is('.' + HOVERSTATE) && hasChildren && that.open(element, true);
                                    }
                                });
                            }
                            else
                    ...
                    if (that.options.openOnClick && that.clicked || mobile) {
                        element.siblings().each(proxy(function (_, sibling) {
                            var $sibling = $(sibling);
                            if (that.options.expandable && !that._isExpanded()) {
                                that.close($sibling, true, true);
                                $sibling.find(that.options.menuTitleSelector).fadeOut();
                                $sibling.animate({
                                    width: that.options.collapsedWidth
                                });
                            }
                            else
                ...
                if (!that.options.openOnClick || !that.clicked) {
                    clearTimeout(that._openTimer);
                    that._openTimer = setTimeout(executeTask, 450);
                } else executeTask();
            ...
            _mouseleave: function (e) {
                ...
                if (!that.options.openOnClick && !touch && !((pointers || msPointers) &&
                    ...
                    if (that.options.expandable && !that._isExpanded()) {
                        hasChildren && that.close(element, false, true);
                        element.find(that.options.menuTitleSelector).fadeOut();
                        element.animate({ width: that.options.collapsedWidth });
                    }
                    else
                        that.close(element);
                }
            ...
            //New fucntionalities implementations
            _isExpanded: function () {
                return this.options.expandable && this.element.is('.' + this.options.expandedClass);
            },
            expand: function () {
                this.element.addClass(this.options.expandedClass);
                this.element.find(itemSelector).removeAttr('style');
                this.element.find(this.options.menuTitleSelector).fadeIn();
            },
            collapse: function () {
                this.element.removeClass(this.options.expandedClass);
                this.element.find(itemSelector).removeAttr('style');
                this.element.find(this.options.menuTitleSelector).fadeOut();
            }
