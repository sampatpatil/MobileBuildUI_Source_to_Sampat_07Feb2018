...
(function (factory) {
    ...
    var NiceScrollClass = function (myopt, me) {
        ...
        //Added by srinath: Gets the options declared using html5 data attributes
        var getDataOptions = function () {
            var d = {}, data = me.data();
            for (var key in data) {
                var k = key.replace('nscroll', '').replace('-', '').toLowerCase();
                for (var gkey in _globaloptions) {
                    if (gkey.toLowerCase() == k) { d[gkey] = data[key]; break; }
                }
            }
            return d;
        };

        $.extend(this.opt, _globaloptions, getDataOptions());
        ...
        this.cancelEvent = function (e) {
            // modified by srinath
            var e = e && (e.original) ? e.original : (e) ? e : window.event || false;
            if (!e) return false;
